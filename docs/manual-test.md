...
1. `A 轮`：点击魔法棒菜单中的 `TT-Agent+`，确认插件全屏工作区打开；关闭工作区后立即在执行任何控制台命令或按 `Tab` 前，确认焦点归还到本轮魔法棒 `TT-Agent+` 触发控件，且 TT 可继续操作。
2. `B 轮`：先聚焦聊天输入框并执行 `/777`，确认打开与 A 轮相同的全屏工作区；关闭工作区后立即在执行任何控制台命令或按 `Tab` 前，确认焦点归还到本轮聊天输入框/触发控件，且 TT 可继续操作。
...
12. 按入口章节上面的 `A 轮`、`B 轮` 分别重复本条检查。每轮先确认打开后焦点进入插件，并按对应入口条目完成关闭后的立即焦点确认；随后执行以下检查，两个结果均应为 `true`。
...
每轮完成上述属性检查后按 `Tab`，再执行以下检查，结果应为 `true`，确认焦点没有进入已关闭的 `panel`。
