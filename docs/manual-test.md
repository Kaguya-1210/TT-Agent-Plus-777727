...
1. `A 轮`：点击魔法棒菜单中的 `TT-Agent+`，确认插件全屏工作区打开；关闭工作区，确认焦点归还到本轮魔法棒 `TT-Agent+` 触发控件，且 TT 可继续操作。
2. `B 轮`：先聚焦聊天输入框并执行 `/777`，确认打开与 A 轮相同的全屏工作区；关闭工作区，确认焦点归还到本轮聊天输入框/触发控件，且 TT 可继续操作。
...
11. 检查整个插件根节点 `#tt-agent-plus-727-root` 及完整页面，确认不存在审核弹窗、灵动岛或对应的空占位。
12. 按入口章节上面的 `A 轮`、`B 轮` 分别重复关闭态与焦点检查。A 轮从魔法棒 `TT-Agent+` 触发控件打开：确认焦点进入插件，关闭后执行以下检查并确认三个结果均为 `true`，再按 `Tab` 确认 `document.activeElement` 不在 `panel` 内，最后确认焦点归还到本轮魔法棒触发控件。B 轮先聚焦聊天输入框并执行 `/777`：确认焦点进入插件，关闭后执行同一段检查并确认三个结果均为 `true`，再按 `Tab` 确认 `document.activeElement` 不在 `panel` 内，最后确认焦点归还到本轮聊天输入框/触发控件。
...
13. 在 `1440x900`、`1280x720`、`390x844`、`360x800`、`844x390`、`720x390`、`721x390` 每个规定尺寸分别执行以下检查，四个结果均应为 `true`。页面、全屏工作区和正文不得横向滚动；仅当视口宽度 `<= 720px` 时，`.ttap-tabs` 才允许 `scrollWidth > clientWidth`，且应能横向滚动访问全部标签。
