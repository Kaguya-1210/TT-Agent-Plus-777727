...
1. `A 轮`：点击魔法棒菜单中的 `TT-Agent+`，确认插件全屏工作区打开；关闭工作区，确认 TT 可继续操作。
2. `B 轮`：聚焦聊天输入框并执行 `/777`，确认打开与 A 轮相同的全屏工作区；关闭工作区，确认 TT 可继续操作。
...
12. 按入口章节定义的 `A 轮`、`B 轮` 分别完整执行以下 5 个步骤：先完成 A 轮的步骤 1-5，再重新打开并完成 B 轮的步骤 1-5，不复用上一轮结果。A 轮的本轮触发控件是魔法棒 `TT-Agent+`；B 轮的本轮触发控件是执行 `/777` 前聚焦的聊天输入框/触发控件。

`步骤 1`：关闭全屏工作区后立即通过页面可见焦点样式确认焦点归还到本轮触发控件；需要使用控制台辅助确认时，必须在按 `Tab` 或点击其他页面控件前检查 `document.activeElement`。

`步骤 2`：在 DevTools 控制台执行以下检查，两个结果均应为 `true`。
